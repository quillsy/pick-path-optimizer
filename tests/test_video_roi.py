...
class TestROICroppingPillow(unittest.TestCase):
    ...
    def test_k_sanitize_source_frame(self):
        from PIL import Image
        import unittest.mock
        path = self._create_image("sanitize1.jpg", 400, 200)
        f = ExtractedFrame(11, 11.0, path, 100, "hash", 400, 200)
        roi = NormalizedROI(0.0, 0.0, 1.0, 1.0)
        
        with unittest.mock.patch('PIL.Image.open') as mock_open:
            mock_open.side_effect = OSError(f"Cannot open {path} for reading")
    ...
    def test_l_sanitize_crop_path(self):
        import unittest.mock
        path = self._create_image("sanitize2.jpg", 400, 200)
        f = ExtractedFrame(12, 12.0, path, 100, "hash", 400, 200)
        roi = NormalizedROI(0.0, 0.0, 1.0, 1.0)
        
        def mock_save(self, fp, *a, **k):
            raise OSError(f"Cannot write to {fp}")
            
        with unittest.mock.patch('PIL.Image.Image.save', new=mock_save):
            with self.assertRaises(VideoROIError) as ctx:
                with temporary_roi_crops([f], roi):
                    pass
    ...
    def test_m_sanitize_temp_dir(self):
        from PIL import Image
        import unittest.mock
        path = self._create_image("sanitize3.jpg", 400, 200)
        f = ExtractedFrame(13, 13.0, path, 100, "hash", 400, 200)
        roi = NormalizedROI(0.0, 0.0, 1.0, 1.0)
        
        # We raise OSError containing self.temp_dir
        with unittest.mock.patch('PIL.Image.open') as mock_open:
            # We don't know the exact temp_dir generated by temporary_roi_crops inside the function,
            # but we can mock tempfile.mkdtemp to return a known one or just raise an error with it.
            # Let's mock tempfile.mkdtemp
            original_mkdtemp = tempfile.mkdtemp
            known_temp_dir = ""
            def mock_mkdtemp(*args, **kwargs):
                nonlocal known_temp_dir
                known_temp_dir = original_mkdtemp(*args, **kwargs)
                return known_temp_dir
                
            with unittest.mock.patch('tempfile.mkdtemp', side_effect=mock_mkdtemp):
                mock_open.side_effect = lambda *a, **k: (_ for _ in ()).throw(OSError(f"Disk full in {known_temp_dir}"))
                with self.assertRaises(VideoROIError) as ctx:
                    with temporary_roi_crops([f], roi):
                        pass
                self.assertNotIn(known_temp_dir, str(ctx.exception))
                self.assertIn("<temp-dir>", str(ctx.exception))
                
            if os.path.exists(known_temp_dir):
                import shutil
                shutil.rmtree(known_temp_dir)

    def test_n_sanitize_truncate(self):
        from PIL import Image
        import unittest.mock
        path = self._create_image("sanitize4.jpg", 400, 200)
        f = ExtractedFrame(14, 14.0, path, 100, "hash", 400, 200)
        roi = NormalizedROI(0.0, 0.0, 1.0, 1.0)
        
        long_err = "X" * 3000
        with unittest.mock.patch('PIL.Image.open') as mock_open:
            mock_open.side_effect = OSError(long_err)
            with self.assertRaises(VideoROIError) as ctx:
                with temporary_roi_crops([f], roi):
                    pass
